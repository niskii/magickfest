...
class OpusTags {
  ...
  constructor(vendorLength: number, vendorString: string, commentLengths: number, comments: Array<string>) {
...
export class OpusFileSplitter {
  ...
  scanPages(buffer: ArrayBufferLike, cb: (a: OggPageHeader) => void) {
    ...
    function readOpusTags(opustagsOffset: number) {
      ...
      position += 4
      let vendorString = decoder.decode(new Uint8Array(view.buffer.slice(position, position + vendorLength)))
      ...
      for (let i = 0; i < n; i++) {
        ...
        position += 4
        totalLength += length
        comments.push(decoder.decode(new Uint8Array(view.buffer.slice(position, position + length))))
        position += length
      }
    ...
    for (let i = 0; i < scanTo; i++) {
      ...
      if (continuedCommentPages && commentPageFound && pageHeader.type.continuedPage == 0) {
      ...
      if (!idPageFound) {
        if (opusIdHeaderMarker === view.getBigUint64(opusOffset)) {
          ...
          idPageFound = true;
          this.#headerObject.preskip = view.getUint16(opusOffset + 10);
