    ...
    #scanPages(buffer: ArrayBufferLike, cb: (a: OggPageHeader) => void) {
        ...
        function readOpusTags(opustagsOffset: number) {
            ...
            let vendorString = decoder.decode(
                new Uint8Array(
                    view.buffer.slice(position, position + vendorLength),
                ),
            );
            ...
            for (let i = 0; i < n; i++) {
                ...
                comments.push(
                    decoder.decode(
                        new Uint8Array(
                            view.buffer.slice(position, position + length),
                        ),
                    ),
                );
                position += length;
        ...
        function printBytes(start: number, length: number) {
            console.log(
                new Uint8Array(view.buffer.slice(start, start + length)),
            );
        }
        ...
        for (let i = 0; i < scanTo; i++) {
            ...
            if (!idPageFound) {
                if (opusIdHeaderMarker === view.getBigUint64(opusOffset)) {
                    ...
                    idPageFound = true;
                    this.#headerObject.preskipGranule = view.getUint16(
                        opusOffset + 10,
                    );
